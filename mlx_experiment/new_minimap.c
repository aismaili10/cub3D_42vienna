...
void draw_rays_on_minimap(t_game *game, double rayDirX, double rayDirY)
{
    ...
    while (1)
    {
        ...
        draw_minimap_pixel(game, mapX, mapY, 0x00FF00); // Green for rays
        printf("mapX: %i, mapY: %i\n", mapX, mapY);
        if (game->map.grid[mapX][mapY] == '1')
            break;

        rayPosX += rayDirX * 0.05; // Increment the ray position slightly
    ...
    for (int x = 0; x < screenWidth; x++)
    {
        ...
        while (hit == 0)
        {
            ...
            if (mapX < 0 || mapX >= game->map.width || mapY < 0 || mapY >= game->map.height)
            {
                ...
                printf("Out of bounds: mapX=%d, mapY=%d\n", mapX, mapY);
            }
            else if (game->map.grid[mapX][mapY] == '1')
            {
                hit = 1; // Wall hit
            }
        }
