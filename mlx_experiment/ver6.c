...
#define ROT_SPEED 0.125         // Rotation speed in radians
...
void draw_rays_on_minimap(t_game *game, double rayDirX, double rayDirY)
{
    ...
    while (1)
    {
        ...
        if (game->map.grid[mapY][mapX] == '1') { // Corrected the index order here
            //printf("WALL HIT\n");
            break;
        }
...
void cast_rays(t_game *game)
{
    for (int x = 0; x < screenWidth; x++)
    {
        ...
        while (hit == 0)
        {
            ...
            else if (game->map.grid[mapY][mapX] == '1') // Corrected the index order here
            {
                hit = 1; // Wall hit
            }
        }
        ...
        if (mapX >= 0 && mapX < game->map.width && mapY >= 0 && mapY < game->map.height)
        {
            switch(game->map.grid[mapY][mapX]) // Corrected the index order here
            {
