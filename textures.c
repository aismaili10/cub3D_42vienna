...
int	load_textures(t_main *cub)
{
	...
	//printf("north->width: %i, north->height: %i\n", cub->north->width, cub->north->height);
...
int	*fill_buffer(t_texture *texture)
{
	...
	while (i < texture->height)
	{
		...
		{
			texture_buff[i * texture->width + j] = *(int *)(texture->addr + (i * texture->line_length + j * (texture->bpp / 8)));
			j++;
		}
		i++;
	}
...
int	create_texture_buffer(t_main *cub)
{
	...
	if (!texture_buff[3])
		return (free(texture_buff[0]), free(texture_buff[1]), free(texture_buff[2]), FAILURE);
